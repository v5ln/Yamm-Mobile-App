...
class BankServices {
  ...
  static const checkIban = '/checkIban';
  ...
  Future<bool> checkIBAN(iban) async{
    var check = false;
    http.Response? response;
    try {
      response = await http.post(
        Uri.parse(baseURL + checkIban),
        body: {
          "iban": iban
        }
      );
    if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        check = (jsonResponse['success']) as bool;
      } else {
        return check;
      }
    } catch (e) {
    ...
    Future<BankList> getDetail(iban) async{
    var details ;
    http.Response? response;
    try {
      response = await http.post(
    ...
    if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        details = (jsonResponse['data']) as BankList;
      } else {
        return details;
      }
    } catch (e) {
